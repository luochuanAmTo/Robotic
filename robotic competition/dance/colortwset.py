        ...
        try:
            ...
            while True:
                ...
                if key == ord('q'):
                    ...
                    if self.speech_enabled:
                        self.tts_proxy.say("")
                elif key == ord('i'):  # i键显示当前状态信息
                    print("="*40)
                    print("当前状态:")
                    print("  语音功能: {}".format("开启" if self.speech_enabled else "关闭"))
                    print("  摄像头订阅: {}".format("已激活" if hasattr(self, 'camera_subscriber') else "未激活"))
                    print("  最近识别的颜色: {}".format(self.last_color))
                    print("="*40)
...
def main():
    ...
    print("重要说明: 机器人将保持当前姿势，不会进行任何移动")
    print("")
    print("使用说明:")
    print("1. 确保机器人摄像头前有物体")
    print("2. 将彩色物体放在十字准星位置")
    print("3. 机器人识别到颜色后会立即说出来")
    print("")
    print("快捷键:")
    print("  'q' - 退出程序")
    print("  '空格' - 手动让机器人说出当前颜色")
    print("  's' - 切换语音开关")
    print("  'i' - 显示当前状态信息")
    print("=" * 60)
