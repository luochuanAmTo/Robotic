...
MODEL_PATH = "/hy-tmp/VLM-R1-main/checkpoints/rl/InternVL3-1B_MPO-rec/checkpoint-54" 
...
# 简化坐标提取函数 - 只提取第一次出现的[[a,b,c,d]]格式
def extract_bbox_answer(content):
    # 直接搜索双括号格式 [[x1,y1,x2,y2]]
    double_bracket_pattern = r'\[\[(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\]\]'
    match = re.search(double_bracket_pattern, content)
