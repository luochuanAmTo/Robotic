...
class LazySupervisedDataset(Dataset):
            ...
            with open(data_path, "r") as file:
                for line in file:
                    ...
                    if 'conversations' in data and len(data['conversations']) >= 2:
                        new_data = {
                            "problem": data['conversations'][0]['value'],
                            "solution": json.dumps(data['conversations'][1]['value']),  # 将坐标列表转为字符串
                            "image": data.get("image", "")
                        }
                        self.list_data_dict.append(new_data)
        
        else:
            raise ValueError(f"Unsupported file type: {data_path}")
        print(f"Total samples loaded: {len(self.list_data_dict)}")
    ...
    def __getitem__(self, i):
        ...
        # 处理图像
        image = None
        if example.get('image') and image_root:
            ...
            return {
                "prompt": [
                    {"role": "system", "content": SYSTEM_PROMPT},
                    {
                        "role": "user",
                        "content": [
                            {"type": "image"} if image else {"type": "text", "text": ""},
        ...
        return {
            ...
            'solution': example['solution'],  # 已经是字符串格式的坐标
...
def main(script_args, training_args, model_args):
    ...
    # Load the dataset
    question_template = "{}"  # 直接使用原始问题，不需要额外模板
