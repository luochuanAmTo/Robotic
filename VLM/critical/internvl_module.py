...
class InvernVLModule(VLMBaseModule):
    ...
    def _load_image(self, image: Image.Image, input_size: int=224, max_num:int=12):
    ...
    def get_question_template(task_type: str):
        match task_type:
            case _:
                return "{Question} output [x1,y1,x2,y2] ."
    ...
    def iou_reward(completions, solution, **kwargs):
        ...
        # 宽松匹配 4 个整数 (允许空格/逗号/中括号/小括号)
        bbox_pattern = r'[-+]?\d+'

        def extract_bbox(text):
            nums = re.findall(bbox_pattern, text)
            if len(nums) >= 4:
                return list(map(int, nums[:4]))  # 取前4个
            return None
        ...
        for i, (content, sol) in enumerate(zip(contents, solution)):
            ...
            try:
                ...
                if isinstance(sol, str):
                    sol_bbox = extract_bbox(sol)
                elif isinstance(sol, (list, tuple)) and len(sol) >= 4:
                    sol_bbox = list(map(int, sol[:4]))

                # --- Pred ---
                pred_bbox = extract_bbox(content)

                if pred_bbox and sol_bbox:
                    reward = iou(pred_bbox, sol_bbox)
